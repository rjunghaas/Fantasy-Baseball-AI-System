...
## Trigger Conditions
- **Sunday run** requires `opponent_roster_YYYYMMDD.parquet` and `opponent_history_YYYYMMDD.parquet` to exist in `data/`
- **Wednesday run** additionally requires `matchup_state_YYYYMMDD.parquet` and the most recent `output_matchup_sunday_*.csv` from a prior Sunday run
- Both runs are manually triggered after the latest data has been pulled

## Workflow

### Sunday Night Run
1. Load data: call `get_opponent_roster()` and `get_opponent_history()` from `data_client.py`
2. Call `get_opponent_tendencies()` from `data_client.py` to get consistency ratings and historical ranges for all 10 categories
3. Apply Decision Rules (Sunday) below to assign a `category_rating` to each category
4. Produce the Sunday output table (all 10 categories)
5. Save output as `data/output_matchup_sunday_YYYYMMDD.csv`

### Wednesday Run
1. Load data: call `get_matchup_state()` and load the most recent `output_matchup_sunday_*.csv`
2. Call `get_category_gaps()` from `data_client.py`
3. Apply Decision Rules (Wednesday) below to assign a `status` to each category
4. Check the 20 IP warning condition (see Decision Rules)
5. Produce the Wednesday output table (all 10 categories), ordered by: `losing_badly` first, then `vulnerable`, then `losing_close`, then `winning_close`, then `winning_comfortably`
6. Save output as `data/output_matchup_wednesday_YYYYMMDD.csv`
...
- `consistency = high` → `category_rating = strong` — opponent wins this category reliably; only contest if you have a clear roster advantage
- `consistency = medium` → `category_rating = medium` — opponent is unpredictable here; worth contesting, especially if you are currently winning it
- `consistency = low` → `category_rating = weak` — opponent struggles here historically; prioritize winning this category
...
| SB | high | 4 | 1 | 3 | 5.5 | 4.5 | True | vulnerable | Connor at 1 SB vs his historical floor of 4 — significant underperformance. I am winning; protect the lead. |
| K | medium | 28 | 24 | 4 | 31.8 | 7.8 | True | vulnerable | Connor below his historical floor in K — real opportunity to extend lead. |
